...
Description: Lasso refitting (Autoregressive covariance with uniformly distributed noises).
"""
...
for i in range(5):
    ...
    for k in range(3):
        ...
        refit_res1.to_csv('./refit_res/refit_AR_d'+str(d)+'_n'+str(n)+'_'+str(job_id)+\
                          '_x'+str(i)+'_beta'+str(k)+'_uniferr.csv', index=False)
