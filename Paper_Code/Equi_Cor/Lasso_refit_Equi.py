...
"""
@author: Yikun Zhang
Last Editing: Jun 13, 2023

Description: Lasso refitting (Equi-correlated covariance with Gaussian noise).
"""
...
from debias_prog import LassoRefit
...
for i in range(5):
    ...
    for k in range(3):
        ...
        X = X_sim[R1 == 1,:].copy()
        Y = Y_sim[R1 == 1].copy()
        m_refit_obs1, asym_var_obs1, sigma_hat_obs1, df = LassoRefit(X, Y, x, method='sqrt_lasso')
        if (i == 0) or (i == 2):
        ...
        X = X_sim[R2 == 1,:].copy()
        Y = Y_sim[R2 == 1].copy()
        m_refit_obs2, asym_var_obs2, sigma_hat_obs2, df = LassoRefit(X, Y, x, method='sqrt_lasso')
        if (i == 0) or (i == 2):
        ...
        m_refit_ipw1, asym_var_ipw1, sigma_hat_ipw1, df = LassoRefit(X, Y, x, method='sqrt_lasso')
        if (i == 0) or (i == 2):
        ...
        m_refit_ipw2, asym_var_ipw2, sigma_hat_ipw2, df = LassoRefit(X, Y, x, method='sqrt_lasso')
        if (i == 0) or (i == 2):
        ...
        ### Full (oracle) data
        m_refit_full, asym_var_full, sigma_hat_full, df = LassoRefit(X_sim, Y_sim, x, method='sqrt_lasso')
        if (i == 0) or (i == 2):
        ...
        refit_res1.to_csv('./refit_res/refit_equicor_d'+str(d)+'_n'+str(n)+'_'+str(job_id)+\
