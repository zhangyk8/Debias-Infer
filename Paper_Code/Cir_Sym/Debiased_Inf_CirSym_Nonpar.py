...
for i in [0,1,2,4]:
    ...
    for k in [0,2]:
        ...
        for non_met in ['NB', 'NBcal', 'RF', 'RFcal', 'SVM', 'SVMcal', 'NN', 'NNcal']:
            if non_met == 'NB':
                lr1_NB = GaussianNB().fit(X_sim, R1)
                prop_score1 = lr1_NB.predict_proba(X_sim)[:,1]
            ...
            if non_met == 'NBcal':
                NB_base = GaussianNB()
                lr1_NB = CalibratedClassifierCV(NB_base, method='sigmoid', cv=5).fit(X_sim, R1)
                prop_score1 = lr1_NB.predict_proba(X_sim)[:,1]
            ...
            if non_met == 'RF':
                lr1_RF = RandomForestClassifier(n_estimators=100, max_depth=None, 
                                                random_state=None, n_jobs=-1).fit(X_sim, R1)
                prop_score1 = lr1_RF.predict_proba(X_sim)[:,1]
            ...
            if non_met == 'RFcal':
                RF_base = RandomForestClassifier(n_estimators=100, max_depth=None, random_state=None, n_jobs=-1)
                lr1_RF = CalibratedClassifierCV(RF_base, method='sigmoid', cv=5).fit(X_sim, R1)
                prop_score1 = lr1_RF.predict_proba(X_sim)[:,1]
            ...
            if non_met == 'SVM':
                lr1_SVM = SVC(kernel='rbf', gamma='scale', probability=True).fit(X_sim, R1)
                prop_score1 = lr1_SVM.predict_proba(X_sim)[:,1]
            ...
            if non_met == 'SVMcal':
                ...
                lr1_SVM = CalibratedClassifierCV(SVM_base, method='sigmoid', cv=5).fit(X_sim, R1)
                prop_score1 = lr1_SVM.predict_proba(X_sim)[:,1]
            ...
            if non_met == 'NN':
                lr1_NN = MLPClassifier(hidden_layer_sizes=(80,50,), activation='relu', 
                                       random_state=None, learning_rate='adaptive', 
                                       learning_rate_init=0.001, max_iter=1000).fit(X_sim, R1)
                prop_score1 = lr1_NN.predict_proba(X_sim)[:,1]
            ...
            if non_met == 'NNcal':
                ...
                lr1_NN = CalibratedClassifierCV(NN_base, method='sigmoid', cv=5).fit(X_sim, R1)
                prop_score1 = lr1_NN.predict_proba(X_sim)[:,1]
            ...
            for rule in para_rule:
                ...
                with open('./debias_res/DebiasProg_CirSym_cov_homoerr_d'+str(d)+'n'+str(n)+'_MCAR'\
                          +str(job_id)+'_x'+str(i)+'_beta'+str(k)+'_prop_'+str(non_met)+'_'+str(rule)+'.dat', "wb") as file:
