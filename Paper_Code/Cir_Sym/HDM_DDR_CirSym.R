...
for (i in 0:5) {
  ...
  for (k in 0:2) {
    ...
    save_result_summary(
      res_mcar,
      file = sprintf("HDM_DDR_res/DDR_CirSym_cov_homoerr_d%dn%d_MCAR%d_x%d_beta%d.csv", d, n, jobid, i, k)
    )

    res_mar <- run_one_setting(X_sim, Y_sim, R2, x, m_true)
    save_result_summary(
      res_mar,
      file = sprintf("HDM_DDR_res/DDR_CirSym_cov_homoerr_d%dn%d_MAR%d_x%d_beta%d.csv", d, n, jobid, i, k)
    )
  }
}
