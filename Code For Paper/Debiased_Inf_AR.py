...
"""
@author: Yikun Zhang
Last Editing: March 26, 2023

Description: Simulations on our debiasing program (1SE, Toeplitz covariance)
"""
...
for i in range(4):
    ...
    for k in range(3):
        ...
        zeta1 = np.logspace(-1, 2, 20)*np.sqrt(np.log(d)/n)
        ...
        zeta2 = np.logspace(-1, 2, 20)*np.sqrt(np.log(d)/n)
        ...
        gamma_n_lst = np.linspace(0.01, np.max(abs(x)), 41)
        cv_fold = 5
        ...
        with open('./debias_res/DebiasProg_AR_cov_homoerr_d'+str(d)+'n'+str(n)+'_MCAR'\
                  +str(job_id)+'_1se_x'+str(i)+'_beta'+str(k)+'.dat', "wb") as file:
